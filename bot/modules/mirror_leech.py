...
def _mirror_leech(
    ...
    def __run_multi():
        ...
        Thread(
            ...
            args=(bot, nextmsg, isZip, extract, isQbit, isLeech, sameDir, isClone),
        ).start()
    ...
    if reply_to:
        ...
        if len(link) == 0 or not is_url(link) and not is_magnet(link):
            if file_ is None:
                ...
                not isQbit
                and file_.mime_type != 'application/x-bittorrent'
                and not isClone
            ):
                ...
                if not isAdmin(message):
                    ...
                    if forcesub(bot, message, tag):
                        return
                    if (maxtask := config_dict['USER_MAX_TASKS']) and check_user_tasks(
                        message.from_user.id, maxtask
                    ):
                        return sendMessage(
                            f'Your tasks limit exceeded for {maxtask} tasks',
                            bot,
                            message,
                        )
                    ...
                    if (
                        not isLeech
                        and drive_id
                        and not GoogleDriveHelper().getFolderData(drive_id)
                    ):
                        return sendMessage(
                            'Google Drive id validation failed!!', bot, message
                        )
                if (
                    dmMode := config_dict['DM_MODE']
                ) and message.chat.type == message.chat.SUPERGROUP:
                    if isLeech and IS_USER_SESSION and not config_dict['DUMP_CHAT']:
                        return sendMessage(
                            'DM_MODE and User Session need DUMP_CHAT', bot, message
                        )
    ...
    if (
        ...
        and not is_Sharerlink(link)
        or (link.isdigit() and multi == 0)
    ):
...
'''.format_map(
            {'cmd': BotCommands.MirrorCommand[0]}
        )
        delete_links(bot, message)
    ...
    if not isAdmin(message):
        ...
        if (maxtask := config_dict['USER_MAX_TASKS']) and check_user_tasks(
            ...
            return sendMessage(
                f'Your tasks limit exceeded for {maxtask} tasks', bot, message
            )
        if isLeech and config_dict['DISABLE_LEECH']:
            delete_links(bot, message)
            return sendMessage('Locked!', bot, message)
    ...
    if (
        ...
        if link.endswith('.torrent') or 'https://api.telegram.org/file/' in link:
            ...
            content_type = get_content_type(link)
        if content_type is None or match(
            r'application/x-bittorrent|application/octet-stream', content_type
        ):
            try:
                resp = request(
                    'GET', link, timeout=10, headers={'user-agent': 'Wget/1.12'}
                )
                if resp.status_code != 200:
                    delete_links(bot, message)
                    sendMessage(
                        f'{tag} ERROR: link got HTTP response: {resp.status_code}',
                        bot,
                        message,
                    )
                    __run_multi()
    ...
    if is_gdrive_link(link):
        ...
        listener.ismega = sendMessage(
            '💡 <b>Mega link this might take a minutes</b>', bot, message
        )
        Thread(
            target=add_mega_download, args=(link, f'{dl_path}/', listener, name)
        ).start()
    elif isQbit and (is_magnet(link) or path.exists(link)):
        Thread(
            target=add_qb_torrent, args=(link, dl_path, listener, ratio, seed_time)
        ).start()
    else:
        mesg = message.text.split('\n')
        if len(mesg) > 1:
        ...
        Thread(
            ...
            args=(link, dl_path, listener, name, auth, ratio, seed_time),
        ).start()
    __run_multi()
